...
import { db, rw_wipe_history } from '../db/db.js';
import { eq, and } from 'drizzle-orm';
import { BM_DATE_FORMAT } from '../utils/bm_scrapper_constants.js';
import { parse_server_attributes, parse_grouped_attributes_for_max, search_for_existing_and_combine, insert_into_db, } from '../helpers/scrapper_generic_helper.js';
class BaseScrapper {
    options;
    all;
    interval;
    ...
    async parse_server_list_data(response) {
        ...
        response.data.map(async (server) => await this.parse_single_server(server));
    ...
    async parse_single_server(server) {
        ...
        let [main_wipe_hour, main_wipe_dow, sec_wipe_hour, sec_wipe_dow, bp_wipe_hour, bp_wipe_dow] = await this.capture_wipe_times(wipe_times);
        ...
        console.log(`Inserting bm_id ${id} with attr keys: [${Object.keys(max_attributes)}] | main_wipe_hour: ${main_wipe_hour} | main_wipe_dow: ${main_wipe_dow} | sec_wipe_hour: ${sec_wipe_hour} | sec_wipe_dow: ${sec_wipe_dow} | bp_wipe_hour: ${bp_wipe_hour} | bp_wipe_dow: ${bp_wipe_dow}`);
        await insert_into_db(final_data_to_insert);
        this.servers_posted++;
    }
    async capture_wipe_times(wipe_times) {
        let main_wipe_hour = '';
        let main_wipe_dow = '';
        let sec_wipe_hour = '';
        let sec_wipe_dow = '';
